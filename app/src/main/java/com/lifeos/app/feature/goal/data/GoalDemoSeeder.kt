...
class GoalDemoSeeder @Inject constructor(
    ...
    override suspend fun seed(profileId: String) {
        val now = Instant.now()
        val goals = if (DemoProfile.fromId(profileId) == DemoProfile.WORK) {
            listOf(
                GoalEntity(
                    ...
                    targetDate = LocalDate.now().plusMonths(2), status = GoalStatus.ACTIVE, linkedHabitIds = emptyList(),
                    linkedJournalIds = emptyList(), categoryId = null, createdAt = now, completedAt = null, profileId = profileId,
                ),
                GoalEntity(
                    id = "${profileId}_goal_2", title = "Get AWS Solutions Architect certified", horizon = GoalHorizon.MONTHLY,
                    description = "Finish the course and pass the exam.",
                    targetDate = LocalDate.now().plusMonths(1), status = GoalStatus.ACTIVE, linkedHabitIds = emptyList(),
                    linkedJournalIds = emptyList(), categoryId = null, createdAt = now, completedAt = null, profileId = profileId,
                ),
                GoalEntity(
                    id = "${profileId}_goal_3", title = "Mentor two junior engineers", horizon = GoalHorizon.LONG_TERM,
                    description = "Run biweekly 1:1s and pair-programming sessions.",
                    targetDate = null, status = GoalStatus.ACTIVE, linkedHabitIds = emptyList(),
                    linkedJournalIds = emptyList(), categoryId = null, createdAt = now, completedAt = null, profileId = profileId,
                ),
            )
        } else {
            listOf(
                GoalEntity(
                    ...
                    targetDate = LocalDate.now().plusMonths(3), status = GoalStatus.ACTIVE, linkedHabitIds = emptyList(),
                    linkedJournalIds = emptyList(), categoryId = null, createdAt = now, completedAt = null, profileId = profileId,
                ),
                GoalEntity(
                ...
                GoalEntity(
                    ...
                    targetDate = LocalDate.now().plusMonths(4), status = GoalStatus.ACTIVE, linkedHabitIds = emptyList(),
                    linkedJournalIds = emptyList(), categoryId = null, createdAt = now, completedAt = null, profileId = profileId,
                ),
            )
        }
